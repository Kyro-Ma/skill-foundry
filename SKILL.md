---
name: skill-demand-agent
description: Discover recent user needs online and turn them into Codex skill folders. Use when Codex should scan public sources and popular Clawhub skills for fresh work, learning, creative, productivity, or happiness-related requirements, create executable skill implementation plans, implement many skills with up to 10 parallel worker agents, review each skill against its plan, publish reviewed skills to ClawHub or GitHub when configured, and produce a markdown catalog of requirements, fit, keywords, and trigger sentences.
---
...
3. Extract and rank requirement-like posts, questions, and popular-skill idea seeds.
4. Corroborate each requirement with at least three separate source signals.
5. Run the requirement scoring agent on every idea, including ideas derived from current Clawhub skills.
6. Remove repeated or same-function ideas; similar ideas can only leave the strongest representative.
...
10. Run a final plan dedupe so same-name or same-function skills only leave one generated skill.
11. Implement skill folders in parallel, capped at 10 worker agents.
...
Run from the workspace root with the bundled or system Python:

```bash
python skill-demand-agent/scripts/run_skill_demand_agent.py --max-ideas 10 --max-workers 10
```
...
Use `--theme-profile office` to focus discovery and scoring on Microsoft Word, Excel, PowerPoint, and cross-Office automation or file-format technical issues while keeping the same implementation and review agents.
...
```bash
python skill-demand-agent/scripts/run_skill_demand_agent.py --publish-clawhub
```
...
```bash
python skill-demand-agent/scripts/run_skill_demand_agent.py --publish-github-repo Kyro-Ma/<repo-name>
```

Use both targets together:

```bash
python skill-demand-agent/scripts/run_skill_demand_agent.py --publish-clawhub --publish-github-repo Kyro-Ma/<repo-name>
```
...
- `generated_skills/<run-id>/`: generated Codex skill folders.
